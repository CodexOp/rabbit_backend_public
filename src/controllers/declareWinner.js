  ...
  try {
    ...
    if (game.winnerAnnouncementTime > 0 && currentTime > game.winnerAnnouncementTime && values.includes(game.announcedWinner)) {
      ...
      const gameSchema = {
        ...
        winnerAnnouncementTime: currentTime + 1000 * 60 * 60 * 24 * 2,
        announcedWinner: 0,
        totalAmount: 0,
      };
...
async function setResult (result) {
  try {
    ...
    if (!game) {
      ...
      console.log('announced winner is: ' + result);
      manageTransactions(result, game.totalAmount * 0.9, gameId);
    ...
    console.log('Error in set result', error);
  }
}
...
module.exports = { checkTime, setResult };
