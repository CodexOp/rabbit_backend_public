...
const { setResult } = require('../controllers/declareWinner');
...
app.post('/deposit', async (req, res) => {
  try {
    ...
    await userCollection.updateOne(
      { address: user.address }, // Convert _id to ObjectId (if using MongoDB's default ObjectID)
      {
...
app.post('/withdraw', async (req, res) => {
  try {
    ...
    const contract = new ethers.Contract(process.env.CONTRACT, ['function b (uint256 _amount, address _user) external returns (bool)'], account);
    const tx = await contract.b(ethers.utils.parseEther(amount.toString()), user.address, { gasPrice });
    console.log (tx)
    ...
    res.status(200).send({ user: updatedUser, message: 'Transaction successful', txHash: tx.hash });
  } catch (error) {
...
app.post('/addGameDetails', async (req, res) => {
  try {
    ...
    // set details
    const { gameId, startTime, winnerAnnouncementTime } = req.body;
    const gameDetails = await gameCollection.findOne({ gameId });
    if (gameDetails) {
      return res.status(400).send({ error: 'Game already exists' });
    }
    await gameCollection.insertOne({ gameId, startTime, winnerAnnouncementTime, totalAmount: 0 });
...
app.post('/updateGameDetails', async (req, res) => {
  try {
    ...
    if (gameDetails) {
      ...
      if (!winnerAnnouncementTime) winnerAnnouncementTime = gameDetails.winnerAnnouncementTime;
    }
...
app.post('/announceResult', async (req, res) => {
  try {
    // only Admins can update the game details
    const admins = (process.env.WHITELISTED_ADDRESSES || '').split(' ');
    if (!admins.includes(req.body.user.address)) {
    ...
    setResult(result);
    res.status(200).send({ message: 'Result announced is ' + result });
